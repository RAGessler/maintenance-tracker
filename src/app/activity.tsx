...
export default function ActivityScreen() {
  ...
  if (adding || editing) return <MaintenanceForm vehicleId={selectedVehicleId} record={editing ?? undefined} onCancel={() => { setAdding(false); setEditing(null); }} onSaved={() => { setAdding(false); setEditing(null); void load(); }} />;
  if (reviewing) return <TripReview trip={reviewing} vehicles={vehicles} onCancel={() => setReviewing(null)} onSaved={() => { setReviewing(null); void load(); }} />;

  const history = buildActivityHistory({ trips, records, readings }, historyVehicleId);
...
function TripReview({ trip, vehicles, onCancel, onSaved }: Readonly<{ trip: Trip; vehicles: GarageVehicle[]; onCancel: () => void; onSaved: () => void }>) {
  ...
  const [saving, setSaving] = useState(false);
  useEffect(() => {
    void maintenanceStore.tracking.getRevisions(trip.id).then(setRevisions).catch(() => setError('Trip revisions could not be loaded.'));
  }, [trip.id]);
  ...
  return <ThemedView style={styles.screen}><SafeAreaView style={styles.safeArea}><ScrollView contentContainerStyle={styles.content}><View style={styles.navigation}><Pressable accessibilityRole="button" onPress={onCancel}><ThemedText type="linkPrimary">Cancel</ThemedText></Pressable><ThemedText accessibilityRole="header" style={styles.sectionTitle}>Review trip</ThemedText><View /></View><TripCard trip={trip} vehicleName={vehicleName(vehicles, trip.vehicleId)} onPress={() => {}} /><View style={styles.form}><Field label="Effective distance (mi)" value={mileage} onChangeText={setMileage} keyboardType="decimal-pad" /><VehiclePicker vehicles={vehicles} selectedVehicleId={vehicleId} onSelect={setVehicleId} label="Assign vehicle" /></View><View style={styles.actions}><ActionButton label="Confirm" disabled={saving || !isMileage(mileage)} onPress={() => void review('confirm')} /><ActionButton label="Save correction" disabled={saving || !isMileage(mileage)} onPress={() => void review('correct')} /><ActionButton label="Reassign" disabled={saving || !vehicleId} onPress={() => void review('reassign')} /><Pressable accessibilityRole="button" disabled={saving} onPress={() => void review('reject')}><ThemedText style={styles.delete}>Reject trip</ThemedText></Pressable></View>{error ? <ThemedText accessibilityLiveRegion="polite" style={styles.error}>{error}</ThemedText> : null}<ThemedText accessibilityRole="header" style={styles.sectionTitle}>Revision history</ThemedText>{revisions.length === 0 ? <ThemedText style={styles.muted}>Loading revisions...</ThemedText> : revisions.map((revision) => <ThemedView key={revision.revisionNumber} style={styles.card}><ThemedText>{revision.action} · {revision.disposition}</ThemedText><ThemedText style={styles.muted}>{formatTimestamp(revision.occurredAt)} · {revision.actor}</ThemedText>{revision.effectiveMilliMiles ? <ThemedText>{formatMilliMiles(revision.effectiveMilliMiles)} mi</ThemedText> : null}</ThemedView>)}</ScrollView></SafeAreaView></ThemedView>;
}

function HistoryCard({ fact, vehicleName, onTrip, onRecord }: Readonly<{ fact: ActivityFact; vehicleName: string; onTrip: () => void; onRecord: () => void }>) {
  if (fact.kind === 'trip') return <TripCard trip={fact.trip} vehicleName={vehicleName} onPress={onTrip} />;
  if (fact.kind === 'record') return <Pressable accessibilityRole="button" accessibilityLabel={`Maintenance record: ${fact.record.serviceName}`} onPress={onRecord}><ThemedView style={styles.card}><ThemedText style={styles.recordTitle}>{fact.record.serviceName}</ThemedText><ThemedText style={styles.muted}>{vehicleName} · {fact.record.completedOn}</ThemedText><ThemedText>{formatMilliMiles(fact.record.milliMiles)} mi</ThemedText></ThemedView></Pressable>;
  return <ThemedView style={styles.card}><ThemedText style={styles.recordTitle}>Manual odometer reading</ThemedText><ThemedText style={styles.muted}>{vehicleName} · {formatTimestamp(fact.reading.effectiveAt)}</ThemedText><ThemedText>{formatMilliMiles(fact.reading.milliMiles)} mi</ThemedText></ThemedView>;
}
